...
function logger(r)
  ...
  --CNH local user              = r.subprocess_env["MAPPED_USER"] -- set by the user mapping code
  --CNH local time_user_map     = r.subprocess_env["OOD_TIME_USER_MAP"] -- set by the user mapping code
  --CNH local time_begin_proxy  = r.subprocess_env["OOD_TIME_BEGIN_PROXY"] -- set by the proxy code
  ...
  if true then
    ...
    -- user
    --CNH msg["local_user"]  = user
    --CNH msg["remote_user"] = r.user
    ...
    -- benchmarks
    --CNH msg["time_proxy"]    = time_begin_proxy and (r:clock() - time_begin_proxy)/1000.0 or 0
    --CNH msg["time_user_map"] = time_user_map and tonumber(time_user_map) or 0
